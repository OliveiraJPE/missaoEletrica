...
function dimensCircCC() {
  ...
  var nCircC = window.document.querySelector("#nCircC").value;
  var nCircTipoCC = window.document.querySelector("#nCircTipoCC").value;
  var nModCC = window.document.querySelector("#nModCC").value;
  var nPotModCC = window.document.querySelector("#nPotModCC").value;
  var nVocModCC = window.document.querySelector("#nVocModCC").value;
  var nVpModCC = window.document.querySelector("#nVpModCC").value;
  var nIscModCC = window.document.querySelector("#nIscModCC").value;
  var nIMaxModCC = window.document.querySelector("#nIMaxModCC").value;
  var Iocpr = window.document.querySelector("#nIocprCC").value;
  var ntrecho1 = window.document.querySelector("#ntrecho1").value;
  var ntrecho2 = window.document.querySelector("#ntrecho2").value;
  var ntrecho3 = window.document.querySelector("#ntrecho3").value;
  var ntrecho4 = window.document.querySelector("#ntrecho4").value;
  var ntrecho5 = window.document.querySelector("#ntrecho5").value;
  var ntrecho6 = window.document.querySelector("#ntrecho6").value;
  var ntrecho7 = window.document.querySelector("#ntrecho7").value;
  var ntrecho8 = window.document.querySelector("#ntrecho8").value;
  
  // Verificando se o nome do Projeto foi dado
  if (!nCircC) {
  ...
  if (nCircTipoCC == 1) {
    // Cálculos
    var potTotalMod = nModCC * nPotModCC;
    var VocTotal = nModCC * nVocModCC;
    var VMaxTotal = nModCC * nVpModCC;
    var IBcc = Number.parseFloat(1.5 * nIscModCC * 1);
    var Scc1 = Number.parseFloat((ntrecho1 * IBcc) / (44 * 0.02 * VMaxTotal));
    var DPS = 1.1 * VocTotal;
  
    // Chamando a função protfio
    var resDisJFio = protfio(IBcc, Scc1);
    var nomeDisj = resDisJFio.DisjProt;
    var nomeFio = resDisJFio.CondFio;
  
   	  /*if (IBcc < 10 && Scc1<=1.5) {
		nomeDisj = Number.parseFloat(10);
		nomeFio = Number.parseFloat(1.5);
	  } else  if (IBcc < 16 && Scc1<=2.5) {
    nomeDisj = Number.parseFloat(16);
    nomeFio = Number.parseFloat(2.5);
    } else if (IBcc < 20 && Scc1<=2.5) {
		nomeDisj = Number.parseFloat(20);
		nomeFio = Number.parseFloat(2.5);
	  } else if (IBcc < 25 && Scc1<=4) {
		nomeDisj = Number.parseFloat(25);
		nomeFio = Number.parseFloat(4);
	  } else if (IBcc < 32 && Scc1<=6) {
		nomeDisj = Number.parseFloat(32);
		nomeFio = Number.parseFloat(6);
	  } else if (IBcc < 40 && Scc1<=10) {
		nomeDisj = Number.parseFloat(40);
		nomeFio = Number.parseFloat(10);
	  } else if (IBcc < 50 && Scc1<=16) {
		nomeDisj = Number.parseFloat(50);
		nomeFio = Number.parseFloat(16);
	  } else if (IBcc < 63 && Scc1<=16) {
		nomeDisj = Number.parseFloat(63);
		nomeFio = Number.parseFloat(16);
	  } else if (IBcc < 80 && Scc1<=25) {
      nomeDisj = Number.parseFloat(80);
      nomeFio = Number.parseFloat(25);
    } else if (IBcc < 100 && Scc1<=35) {
		nomeDisj = Number.parseFloat(100);
		nomeFio = Number.parseFloat(35);
	  } else {
		alert(
		  "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que  a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
		);
		nomeDisj = Number.parseFloat(0);
		nomeFio = Number.parseFloat(0);
	  }*/
/********************************** */
   if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
     if((1.5*nIscModCC<nomeDisj<2.4*nIscModCC) && (nomeDisj<Iocpr)){
      alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj}A.` );
      }else{
        alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
      }
    }else{
      alert(`Esta série(séries) não necessitam de proteção. O seccionador da StringBox deve suportar no mínimo ${DPS.toFixed(0)}V, e  ${nomeDisj}A.`);
    }
   /************************************************************* */ 
    
  
	  if(nomeDisj==0||nomeFio==0){
		  alert("ERRO: Condutor superou 35mm² em, pelo menos, um dos critérios de dimensionamento. Consulte um técnico ou refaça o dimensionamento.")}
		  else{

        alert( `ATENÇÃO: Segue as NBR's 16612, 16690 e 5410. \n-->Este dimensionamento considera condutores e cabos solares, no modo de instalação - M1: dois cabos unipolares encostados um ao outro na horizontal, instalados ao ar livre exposta ao sol, 50ºC(ar) e temperatura do condutor em regime permanente de 90ºC -  Tabela C.4.(NBR16612), até 35mm² e máxima queda de tensão admissível entre o quadro e a carga de 2%. \n-->Já o dimensionamento considera o nº idêntico de módulos em cada string em paralelo, os critérios de máxima corrente (Tabela C.4. NBR16612), e o critério de máxima queda de tensão admissível - S= (L x IB)/(44 x e), (Ver apostila solar).  \n-->UTILIZE O MAIOR CONDUTOR ENCONTRADO.`); 
        alert(`Sistema ${nCircC}, com ${potTotalMod}Wp, corrente de projeto IB1 = ${IBcc.toFixed(2)}A,  com L1 = ${ntrecho1} metros. \n-->O condutor para trecho 01 de ${nomeFio} mm². \n-->O DPS deve ter valor maior do que ${DPS.toFixed(0)}V, Classe II. \n-->OBS.:Em caso de possibilidade de descagas atmosféricas, ou distância entre o sistema e o quadro CC maior do que 10m, consulte um Técnico ou Engenheiro para o dimensionamento do DPS e Sistema de aterramento.`);
      
    }
 }

 function protfio(IBcc, Scc) {
  if (IBcc < 10 && Scc <= 1.5) {
    return { DisjProt: Number.parseFloat(10), CondFio: Number.parseFloat(1.5) };
  } else if (IBcc < 16 && Scc <= 2.5) {
    return { DisjProt: Number.parseFloat(16), CondFio: Number.parseFloat(2.5) };
  } else if (IBcc < 20 && Scc <= 2.5) {
    return { DisjProt: Number.parseFloat(20), CondFio: Number.parseFloat(2.5) };
  } else if (IBcc < 25 && Scc <= 4) {
    return { DisjProt: Number.parseFloat(25), CondFio: Number.parseFloat(4) };
  } else if (IBcc < 32 && Scc <= 6) {
    return { DisjProt: Number.parseFloat(32), CondFio: Number.parseFloat(6) };
  } else if (IBcc < 40 && Scc <= 10) {
    return { DisjProt: Number.parseFloat(40), CondFio: Number.parseFloat(10) };
  } else if (IBcc < 50 && Scc <= 16) {
    return { DisjProt: Number.parseFloat(50), CondFio: Number.parseFloat(16) };
  } else if (IBcc < 63 && Scc <= 16) {
    return { DisjProt: Number.parseFloat(63), CondFio: Number.parseFloat(16) };
  } else if (IBcc < 80 && Scc <= 25) {
    return { DisjProt: Number.parseFloat(80), CondFio: Number.parseFloat(25) };
  } else if (IBcc < 100 && Scc <= 35) {
    return { DisjProt: Number.parseFloat(100), CondFio: Number.parseFloat(35) };
  } else {
    alert(
      "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
    );
 ...
 if (nCircTipoCC == 2) {
  ...
  IBcc = 1.5*nIscModCC*1;
  IBcc1 = 1.5*nIscModCC*1;
  IBcc2 = 1.5*nIscModCC*1;
  Scc1 = (ntrecho1*IBcc1)/(44*0.02*VMaxTotal);
  Scc2 = (ntrecho2*IBcc2)/(44*0.02*VMaxTotal);
  DPS = 1.1* VocTotal;

  if (IBcc1 < 10 && Scc1<=1.5) {
		nomeDisj1 = Number.parseFloat(10);
		nomeFio1 = Number.parseFloat(1.5);
	  } else  if (IBcc1 < 16 && Scc1<=2.5) {
    nomeDisj1 = Number.parseFloat(16);
    nomeFio1 = Number.parseFloat(2.5);
    } else if (IBcc1 < 20 && Scc1<=2.5) {
		nomeDisj1 = Number.parseFloat(20);
		nomeFio1 = Number.parseFloat(2.5);
	  } else if (IBcc1 < 25 && Scc1<=4) {
		nomeDisj1 = Number.parseFloat(25);
		nomeFio1 = Number.parseFloat(4);
	  } else if (IBcc1 < 32 && Scc1<=6) {
		nomeDisj1 = Number.parseFloat(32);
		nomeFio1 = Number.parseFloat(6);
	  } else if (IBcc1 < 40 && Scc1<=10) {
		nomeDisj1 = Number.parseFloat(40);
		nomeFio1 = Number.parseFloat(10);
	  } else if (IBcc1 < 50 && Scc1<=16) {
		nomeDisj1 = Number.parseFloat(50);
		nomeFio1 = Number.parseFloat(16);
	  } else if (IBcc1 < 63 && Scc1<=16) {
		nomeDisj1 = Number.parseFloat(63);
		nomeFio1 = Number.parseFloat(16);
	  } else if (IBcc1 < 80 && Scc1<=25) {
      nomeDisj1 = Number.parseFloat(80);
      nomeFio1 = Number.parseFloat(25);
    } else if (IBcc1 < 100 && Scc1<=35) {
		nomeDisj1 = Number.parseFloat(100);
		nomeFio1 = Number.parseFloat(35);
	  }  else {
		alert(
		  "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que  a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
		);
		nomeDisj1 = Number.parseFloat(0);
		nomeFio1 = Number.parseFloat(0);
	  }
  /********************************** */
  if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
    if((1.5*nIscModCC<nomeDisj1<2.4*nIscModCC) && (nomeDisj1<Iocpr)){
       alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj1}A.` );
     }else{
       alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.");       
     }
    }else{
      alert(`Esta série(séries) não necessitam de proteção. O seccionador da StringBox deve suportar no mínimo ${DPS.toFixed(0)}V, e  ${nomeDisj1}A.`);
    }
  /************************************************************* */ 
   
	 
  if (IBcc2 < 10 && Scc2<=1.5) {
		nomeDisj2 = Number.parseFloat(10);
		nomeFio2 = Number.parseFloat(1.5);
	  } else  if (IBcc2 < 16 && Scc2<=2.5) {
    nomeDisj2 = Number.parseFloat(16);
    nomeFio2 = Number.parseFloat(2.5);
    } else if (IBcc2 < 20 && Scc2<=2.5) {
		nomeDisj2 = Number.parseFloat(20);
		nomeFio2 = Number.parseFloat(2.5);
	  } else if (IBcc2 < 25 && Scc2<=4) {
		nomeDisj2 = Number.parseFloat(25);
		nomeFio2 = Number.parseFloat(4);
	  } else if (IBcc2 < 32 && Scc2<=6) {
		nomeDisj2 = Number.parseFloat(32);
		nomeFio2 = Number.parseFloat(6);
	  } else if (IBcc2 < 40 && Scc2<=10) {
		nomeDisj2 = Number.parseFloat(40);
		nomeFio2 = Number.parseFloat(10);
	  } else if (IBcc2 < 50 && Scc2<=16) {
		nomeDisj2 = Number.parseFloat(50);
		nomeFio2 = Number.parseFloat(16);
	  } else if (IBcc2 < 63 && Scc2<=16) {
		nomeDisj2 = Number.parseFloat(63);
		nomeFio2 = Number.parseFloat(16);
	  } else if (IBcc2 < 80 && Scc2<=25) {
      nomeDisj2 = Number.parseFloat(80);
      nomeFio2 = Number.parseFloat(25);
    } else if (IBcc2 < 100 && Scc2<=35) {
		nomeDisj2 = Number.parseFloat(100);
		nomeFio2 = Number.parseFloat(35);
	  }  else {
		alert(
		  "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que  a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
		);
		nomeDisj2 = Number.parseFloat(0);
		nomeFio2 = Number.parseFloat(0);
	  }
    /********************************** */
   if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
    if((1.5*nIscModCC<nomeDisj2<2.4*nIscModCC) && (nomeDisj2<Iocpr)){
      alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj2}A.` );
     }else{
      alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
     }
    }else{
      alert(`Esta série(séries) não necessitam de proteção. O seccionador da StringBox deve suportar no mínimo ${DPS.toFixed(0)}V, e  ${nomeDisj2}A.`);
    }
  /************************************************************* */ 
   
    if(nomeDisj1==0||nomeFio1==0 || nomeDisj2==0||nomeFio2==0){
		  alert("ERRO: Condutor superou 35mm² em, pelo menos, um dos critérios de dimensionamento. Consulte um técnico ou refaça o dimensionamento.")}
		  else{
	  
    
        alert( `ATENÇÃO: Segue as NBR's 16612, 16690 e 5410. \n-->Este dimensionamento considera condutores e cabos solares, no modo de instalação - M1: dois cabos unipolares encostados um ao outro na horizontal, instalados ao ar livre exposta ao sol, 50ºC(ar) e temperatura do condutor em regime permanente de 90ºC -  Tabela C.4.(NBR16612), até 35mm² e máxima queda de tensão admissível entre o quadro e a carga de 2%. \n-->Já o dimensionamento considera o nº idêntico de módulos em cada string em paralelo, os critérios de máxima corrente (Tabela C.4. NBR16612), e o critério de máxima queda de tensão admissível - S= (L x IB)/(44 x e), (Ver apostila solar).  \n-->UTILIZE O MAIOR CONDUTOR ENCONTRADO.`); 
        alert(`Sistema ${nCircC}, com ${potTotalMod}Wp, corrente de projeto IB1 = ${IBcc1.toFixed(2)}A,  com L1 = ${ntrecho1} metros, corrente de projeto IB2 = ${IBcc2.toFixed(2)}A,  com L2 = ${ntrecho2} metros. \n-->O condutor para trecho 01 de ${nomeFio1} mm²; para o trecho 02 de ${nomeFio2} mm².  \n-->O DPS deve ter valor maior do que ${DPS.toFixed(0)}V, Classe II. \n-->OBS.:Em caso de possibilidade de descagas atmosféricas, ou distância entre o sistema e o quadro CC maior do que 10m, consulte um Técnico ou Engenheiro para o dimensionamento do DPS e Sistema de aterramento.`);
...
if (nCircTipoCC == 3) {
  ...
  VocTotal = nModCC*nVocModCC;
  VMaxTotal = nModCC*nVpModCC
  IBcc1 = 1.25*nIscModCC*2;
  IBcc2 = 1.25*nIscModCC*2;
  IBcc3 = IBcc1+IBcc2;
  Scc1 = (ntrecho1*IBcc1)/(44*0.02*VMaxTotal);
  Scc2 = (ntrecho2*IBcc2)/(44*0.02*VMaxTotal);
  Scc3 = (ntrecho3*IBcc3)/(44*0.01*VMaxTotal);
  if(Scc1>Scc2 && Scc1>=Scc3){
    Scc3=Scc1+1;
  }
  if(Scc2>Scc1 && Scc2>=Scc3){
    Scc3=Scc2+1;
  }
  if(Scc1===Scc2 && Scc1>=Scc3){
    Scc3=Scc1+1;
  ...
  DPS = 1.1* VocTotal;
    ...
    /********************************** */
   if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
    if((1.25*nIscModCC<nomeDisj1<2.4*nIscModCC) && (nomeDisj1<Iocpr)){
      alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj1}A.` );
     }else{
      alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
     }
  ...
  /************************************************************* */ 
    ...
    if (IBcc2 < 10 && Scc2<=1.5) {
      ...
      if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
        if((1.25*nIscModCC<nomeDisj2<2.4*nIscModCC) && (nomeDisj2<Iocpr)){
          alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj2}A.` );
         }else{
          alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
         }
      ...
      if (IBcc3 < 10 && Scc3<=1.5) {
        nomeDisj3 = Number.parseFloat(10);
        nomeFio3 = Number.parseFloat(1.5);
        } else  if (IBcc3 < 16 && Scc3<=2.5) {
        nomeDisj3 = Number.parseFloat(16);
        nomeFio3 = Number.parseFloat(2.5);
        } else if (IBcc3 < 20 && Scc3<=2.5) {
        nomeDisj3 = Number.parseFloat(20);
        nomeFio3 = Number.parseFloat(2.5);
        } else if (IBcc3 < 25 && Scc3<=4) {
        nomeDisj3 = Number.parseFloat(25);
        nomeFio3 = Number.parseFloat(4);
        } else if (IBcc3 < 32 && Scc3<=6) {
        nomeDisj3 = Number.parseFloat(32);
        nomeFio3 = Number.parseFloat(6);
        } else if (IBcc3 < 40 && Scc3<=10) {
        nomeDisj3 = Number.parseFloat(40);
        nomeFio3 = Number.parseFloat(10);
        } else if (IBcc3 < 50 && Scc3<=16) {
        nomeDisj3 = Number.parseFloat(50);
        nomeFio3 = Number.parseFloat(16);
        } else if (IBcc3 < 63 && Scc3<=16) {
        nomeDisj3 = Number.parseFloat(63);
        nomeFio3 = Number.parseFloat(16);
        } else if (IBcc3 < 80 && Scc3<=25) {
          nomeDisj3 = Number.parseFloat(80);
          nomeFio3 = Number.parseFloat(25);
        } else if (IBcc3 < 100 && Scc3<=35) {
        nomeDisj3 = Number.parseFloat(100);
        nomeFio3 = Number.parseFloat(35);
        }  else {
        alert(
          "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que  a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
        );
        nomeDisj3 = Number.parseFloat(0);
        nomeFio3 = Number.parseFloat(0);
        }

        if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
          if((1.25*nIscModCC<nomeDisj3<2.4*nIscModCC) && (nomeDisj3<Iocpr)){
            alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj3}A.` );
           }else{
            alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
           }
          }else{
            alert(`Esta série(séries) não necessitam de proteção. O seccionador da StringBox deve suportar no mínimo ${DPS.toFixed(0)}V, e  ${nomeDisj3}A.`);
          }

      if(nomeDisj1==0||nomeFio1==0 || nomeDisj2==0||nomeFio2==0 || nomeDisj3==0||nomeFio3==0 ){
        alert("ERRO: Condutor superou 35mm² em, pelo menos, um dos critérios de dimensionamento. Consulte um técnico ou refaça o dimensionamento.")}
        else{


          alert( `ATENÇÃO: Segue as NBR's 16612, 16690 e 5410. \n-->Este dimensionamento considera condutores e cabos solares, no modo de instalação - M1: dois cabos unipolares encostados um ao outro na horizontal, instalados ao ar livre exposta ao sol, 50ºC(ar) e temperatura do condutor em regime permanente de 90ºC -  Tabela C.4.(NBR16612), até 35mm² e máxima queda de tensão admissível entre o quadro e a carga de 2%. \n-->Já o dimensionamento considera o nº idêntico de módulos em cada string em paralelo, os critérios de máxima corrente (Tabela C.4. NBR16612), e o critério de máxima queda de tensão admissível - S= (L x IB)/(44 x e), (Ver apostila solar).  \n-->UTILIZE O MAIOR CONDUTOR ENCONTRADO.`); 
          alert(`Sistema ${nCircC}, com ${potTotalMod}Wp, corrente de projeto IB1 = ${IBcc1.toFixed(2)}A,  com L1 = ${ntrecho1} metros, corrente de projeto IB2 = ${IBcc2.toFixed(2)}A,  com L2 = ${ntrecho2} metros, corrente de projeto IB3 = ${IBcc3.toFixed(2)}A,  com L3 = ${ntrecho3} metros. \n-->O condutor para trecho 01 de ${nomeFio1} mm²; para o trecho 02 de ${nomeFio2} mm², o condutor para trecho 3 de ${nomeFio3} mm².  \n-->O DPS deve ter valor maior do que ${DPS.toFixed(0)}V, Classe II. \n-->OBS.:Em caso de possibilidade de descagas atmosféricas, ou distância entre o sistema e o quadro CC maior do que 10m, consulte um Técnico ou Engenheiro para o dimensionamento do DPS e Sistema de aterramento.`);
...
if (nCircTipoCC == 4) {
  ...
  VocTotal = nModCC*nVocModCC;
  VMaxTotal = nModCC*nVpModCC
  IBcc1 = IBcc2 = IBcc3 = IBcc4 = 1.25*nIscModCC*2;
  IBcc5 = IBcc6 = IBcc1*2;
  IBcc7 = IBcc8 = IBcc5*2;
...
//IBcc5
  Scc1 = (ntrecho1*IBcc1)/(44*0.02*VMaxTotal);
  Scc2 = (ntrecho2*IBcc2)/(44*0.02*VMaxTotal);
  Scc5 = (ntrecho5*IBcc5)/(44*0.02*VMaxTotal);

if(Scc1>Scc2 && Scc1>=Scc5){
    Scc5=Scc1+1;
  }
  if(Scc2>Scc1 && Scc2>=Scc5){
    Scc5=Scc2+1;
  }
  if(Scc1===Scc2 && Scc1>=Scc5){
    Scc5=Scc1+1;
...
//Ibcc6
  Scc3 = (ntrecho3*IBcc3)/(44*0.01*VMaxTotal);
  Scc4 = (ntrecho4*IBcc4)/(44*0.02*VMaxTotal);
  Scc6 = (ntrecho6*IBcc6)/(44*0.01*VMaxTotal);

 if(Scc3>Scc4 && Scc3>=Scc6){
    Scc5=Scc3+1;
  }
  if(Scc4>Scc3 && Scc4>=Scc6){
    Scc6=Scc4+1;
  }
  if(Scc3===Scc4 && Scc3>=Scc6){
    Scc6=Scc3+1;
...
//IBcc7
  Scc7 = (ntrecho7*IBcc7)/(44*0.02*VMaxTotal);
  if(Scc5>Scc6 && Scc5>=Scc7){
    Scc7=Scc5+1;
  }
  if(Scc6>Scc5 && Scc6>=Scc7){
    Scc7=Scc6+1;
  }
  if(Scc6===Scc5 && Scc5>=Scc7){
    Scc7=Scc5+1;
  }
  Scc8 = (ntrecho8*IBcc8)/(44*0.02*VMaxTotal);
  ...
  DPS = 1.1* VocTotal;
    ...
    if (IBcc2 < 10 && Scc2<=1.5) {
      ...
      if (IBcc3 < 10 && Scc3<=1.5) {
        ...
        if (IBcc4 < 10 && Scc4<=1.5) {
          nomeDisj4 = Number.parseFloat(10);
          nomeFio4 = Number.parseFloat(1.5);
          } else  if (IBcc4 < 16 && Scc4<=2.5) {
          nomeDisj4 = Number.parseFloat(16);
          nomeFio4 = Number.parseFloat(2.5);
          } else if (IBcc4 < 20 && Scc4<=2.5) {
          nomeDisj4 = Number.parseFloat(20);
          nomeFio4 = Number.parseFloat(2.5);
          } else if (IBcc4 < 25 && Scc4<=4) {
          nomeDisj4 = Number.parseFloat(25);
          nomeFio4 = Number.parseFloat(4);
          } else if (IBcc4 < 32 && Scc4<=6) {
          nomeDisj4 = Number.parseFloat(32);
          nomeFio4 = Number.parseFloat(6);
          } else if (IBcc4 < 40 && Scc4<=10) {
          nomeDisj4 = Number.parseFloat(40);
          nomeFio4 = Number.parseFloat(10);
          } else if (IBcc4 < 50 && Scc4<=16) {
          nomeDisj4 = Number.parseFloat(50);
          nomeFio4 = Number.parseFloat(16);
          } else if (IBcc4 < 63 && Scc4<=16) {
          nomeDisj4 = Number.parseFloat(63);
          nomeFio4 = Number.parseFloat(16);
          } else if (IBcc4 < 80 && Scc4<=25) {
            nomeDisj4 = Number.parseFloat(80);
            nomeFio4 = Number.parseFloat(25);
          } else if (IBcc4 < 100 && Scc4<=35) {
          nomeDisj4 = Number.parseFloat(100);
          nomeFio4 = Number.parseFloat(35);
          }  else {
          alert(
            "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que  a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
          );
          nomeDisj4 = Number.parseFloat(0);
          nomeFio4 = Number.parseFloat(0);
          }
          if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
            if((1.25*nIscModCC<nomeDisj4<2.4*nIscModCC) && (nomeDisj4<Iocpr)){
              alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj4}A.` );
             }else{
              alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
             }
            }else{
              alert(`Esta série(séries) não necessitam de proteção. O seccionador da StringBox deve suportar no mínimo ${DPS.toFixed(0)}V, e  ${nomeDisj4}A.`);
            }
         
          if (IBcc5 < 10 && Scc5<=1.5) {
            nomeDisj5 = Number.parseFloat(10);
            nomeFio5 = Number.parseFloat(1.5);
            } else  if (IBcc5 < 16 && Scc5<=2.5) {
            nomeDisj5 = Number.parseFloat(16);
            nomeFio5 = Number.parseFloat(2.5);
            } else if (IBcc5 < 20 && Scc5<=2.5) {
            nomeDisj5 = Number.parseFloat(20);
            nomeFio5 = Number.parseFloat(2.5);
            } else if (IBcc5 < 25 && Scc5<=4) {
            nomeDisj5 = Number.parseFloat(25);
            nomeFio5 = Number.parseFloat(4);
            } else if (IBcc5 < 32 && Scc5<=6) {
            nomeDisj5 = Number.parseFloat(32);
            nomeFio5 = Number.parseFloat(6);
            } else if (IBcc5 < 40 && Scc5<=10) {
            nomeDisj5 = Number.parseFloat(40);
            nomeFio5 = Number.parseFloat(10);
            } else if (IBcc5 < 50 && Scc5<=16) {
            nomeDisj5 = Number.parseFloat(50);
            nomeFio5 = Number.parseFloat(16);
            } else if (IBcc5 < 63 && Scc5<=16) {
            nomeDisj5 = Number.parseFloat(63);
            nomeFio5 = Number.parseFloat(16);
            } else if (IBcc5 < 80 && Scc5<=25) {
              nomeDisj5 = Number.parseFloat(80);
              nomeFio5 = Number.parseFloat(25);
            } else if (IBcc5 < 100 && Scc5<=35) {
            nomeDisj5 = Number.parseFloat(100);
            nomeFio5 = Number.parseFloat(35);
            }  else {
            alert(
              "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que  a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
            );
            nomeDisj5 = Number.parseFloat(0);
            nomeFio5 = Number.parseFloat(0);
            }
            if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
              if((1.25*nIscModCC<nomeDisj5<2.4*nIscModCC) && (nomeDisj5<Iocpr)){
                alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj5}A.` );
               }else{
                alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
               }
              }else{
                alert(`Esta série(séries) não necessitam de proteção. O seccionador da StringBox deve suportar no mínimo ${DPS.toFixed(0)}V, e  ${nomeDisj5}A.`);
              }
            if (IBcc6 < 10 && Scc6<=1.5) {
              nomeDisj6 = Number.parseFloat(10);
              nomeFio6 = Number.parseFloat(1.5);
              } else  if (IBcc6 < 16 && Scc6<=2.5) {
              nomeDisj6 = Number.parseFloat(16);
              nomeFio6 = Number.parseFloat(2.5);
              } else if (IBcc6 < 20 && Scc6<=2.5) {
              nomeDisj6 = Number.parseFloat(20);
              nomeFio6 = Number.parseFloat(2.5);
              } else if (IBcc6 < 25 && Scc6<=4) {
              nomeDisj6 = Number.parseFloat(25);
              nomeFio6 = Number.parseFloat(4);
              } else if (IBcc6 < 32 && Scc6<=6) {
              nomeDisj6 = Number.parseFloat(32);
              nomeFio6 = Number.parseFloat(6);
              } else if (IBcc6 < 40 && Scc6<=10) {
              nomeDisj6 = Number.parseFloat(40);
              nomeFio6 = Number.parseFloat(10);
              } else if (IBcc6 < 50 && Scc6<=16) {
              nomeDisj6 = Number.parseFloat(50);
              nomeFio6 = Number.parseFloat(16);
              } else if (IBcc6 < 63 && Scc6<=16) {
              nomeDisj6 = Number.parseFloat(63);
              nomeFio6 = Number.parseFloat(16);
              } else if (IBcc6 < 80 && Scc6<=25) {
                nomeDisj6 = Number.parseFloat(80);
                nomeFio6 = Number.parseFloat(25);
              } else if (IBcc6 < 100 && Scc6<=35) {
              nomeDisj6 = Number.parseFloat(100);
              nomeFio6 = Number.parseFloat(35);
              }  else {
              alert(
                "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que  a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
              );
              nomeDisj6 = Number.parseFloat(0);
              nomeFio6 = Number.parseFloat(0);
              }
              if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
                if((1.25*nIscModCC<nomeDisj6<2.4*nIscModCC) && (nomeDisj6<Iocpr)){
                  alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj6}A.` );
                 }else{
                  alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
                 }
                }else{
                  alert(`Esta série(séries) não necessitam de proteção. O seccionador da StringBox deve suportar no mínimo ${DPS.toFixed(0)}V, e  ${nomeDisj6}A.`);
                }

              if (IBcc7 < 10 && Scc7<=1.5) {
                nomeDisj7 = Number.parseFloat(10);
                nomeFio7 = Number.parseFloat(1.5);
                } else  if (IBcc7 < 16 && Scc7<=2.5) {
                nomeDisj7 = Number.parseFloat(16);
                nomeFio7 = Number.parseFloat(2.5);
                } else if (IBcc7 < 20 && Scc7<=2.5) {
                nomeDisj7 = Number.parseFloat(20);
                nomeFio7 = Number.parseFloat(2.5);
                } else if (IBcc7 < 25 && Scc7<=4) {
                nomeDisj7 = Number.parseFloat(25);
                nomeFio7 = Number.parseFloat(4);
                } else if (IBcc7 < 32 && Scc7<=6) {
                nomeDisj7 = Number.parseFloat(32);
                nomeFio7 = Number.parseFloat(6);
                } else if (IBcc7 < 40 && Scc7<=10) {
                nomeDisj7 = Number.parseFloat(40);
                nomeFio7 = Number.parseFloat(10);
                } else if (IBcc7 < 50 && Scc7<=16) {
                nomeDisj7 = Number.parseFloat(50);
                nomeFio7 = Number.parseFloat(16);
                } else if (IBcc7 < 63 && Scc7<=16) {
                nomeDisj7 = Number.parseFloat(63);
                nomeFio7 = Number.parseFloat(16);
                } else if (IBcc7 < 80 && Scc7<=25) {
                  nomeDisj7 = Number.parseFloat(80);
                  nomeFio7 = Number.parseFloat(25);
                } else if (IBcc7 < 100 && Scc7<=35) {
                nomeDisj7 = Number.parseFloat(100);
                nomeFio7 = Number.parseFloat(35);
                }  else {
                alert(
                  "ERRO: Ampacidade - Condutor maior do que 35mm², procure um Técnico ou refaça a operação ou Disjuntor maior do que  a capacidade do condutor de 35mm², procure um Técnico ou refaça a operação"
                );
                nomeDisj7 = Number.parseFloat(0);
                nomeFio7 = Number.parseFloat(0);
                }
               if ((Number.parseFloat(nCircTipoCC)-1)*Number.parseFloat(nIscModCC) > Number.parseFloat(Iocpr)){
    if((1.25*nIscModCC<nomeDisj7<2.4*nIscModCC) && (nomeDisj7<Iocpr)){
      alert(`Disjuntor ou Fusível gPV (com seccionador ou não) deve ser de ${nomeDisj7}A.` );
     }else{
      alert("Disjuntor ou Fusível gPV não é possível de ser calculado a partir do relecionamento dos parametros inseridos.")
     }
   }else{
    alert(`Esta série(séries) não necessitam de proteção. O seccionador da StringBox deve suportar no mínimo ${DPS.toFixed(0)}V, e  ${nomeDisj7}A.`);
  }
          
            /******* */
      if(nomeDisj1==0||nomeFio1==0 || nomeDisj2==0||nomeFio2==0 || nomeDisj3==0||nomeFio3==0 || nomeDisj4==0||nomeFio4==0 || nomeDisj5==0||nomeFio5==0 || nomeDisj6==0||nomeFio6==0 || nomeDisj7==0||nomeFio7==0){
        alert("ERRO: Condutor superou 35mm² em, pelo menos, um dos critérios de dimensionamento. Consulte um técnico ou refaça o dimensionamento.")}
        else{
      alert( `ATENÇÃO: Segue as NBR's 16612, 16690 e 5410. \n-->Este dimensionamento considera condutores e cabos solares, no modo de instalação - M1: dois cabos unipolares encostados um ao outro na horizontal, instalados ao ar livre exposta ao sol, 50ºC(ar) e temperatura do condutor em regime permanente de 90ºC -  Tabela C.4.(NBR16612), até 35mm² e máxima queda de tensão admissível entre o quadro e a carga de 2%. \n-->Já o dimensionamento considera o nº idêntico de módulos em cada string em paralelo, os critérios de máxima corrente (Tabela C.4. NBR16612), e o critério de máxima queda de tensão admissível - S= (L x IB)/(44 x e), (Ver apostila solar).  \n-->UTILIZE O MAIOR CONDUTOR ENCONTRADO.`); 
      alert(`Sistema ${nCircC}, com ${potTotalMod}Wp, corrente de projeto IB1 = ${IBcc1.toFixed(2)}A,  com L1 = ${ntrecho1} metros, corrente de projeto IB2 = ${IBcc2.toFixed(2)}A,  com L2 = ${ntrecho2} metros, corrente de projeto IB3 = ${IBcc3.toFixed(2)}A,  com L3 = ${ntrecho3} metros, corrente de projeto IB4 = ${IBcc4.toFixed(2)}A,  com L4 = ${ntrecho4} metros, a corrente de projeto IB5 = ${IBcc5.toFixed(2)}A,  com L5 = ${ntrecho5} metros, corrente de projeto IB6 = ${IBcc6.toFixed(2)}A,  com L6 = ${ntrecho6} metros, e a corrente de projeto IB7 = ${IBcc7.toFixed(2)}A,  com L7 = ${ntrecho7} metros. \n-->O condutor para trecho 01 de ${nomeFio1} mm²; para o trecho 02 de ${nomeFio2} mm², o condutor para trecho 3 de ${nomeFio3} mm², o condutor para o trecho 04 de ${nomeFio4} mm², para o trecho 05 de ${nomeFio5} mm², o condutor para o trecho 06 de ${nomeFio6}mm², e o condutor para o trecho 07 de ${nomeFio7} mm².  \n-->O DPS deve ter valor maior do que ${DPS.toFixed(0)}V, Classe II. \n-->OBS.:Em caso de possibilidade de descagas atmosféricas, ou distância entre o sistema e o quadro CC maior do que 10m, consulte um Técnico ou Engenheiro para o dimensionamento do DPS e Sistema de aterramento.`);
